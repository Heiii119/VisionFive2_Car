...
# How fast values change while you HOLD a button (web sends booleans at 60Hz)
STEP = 5
STEERING_STEP = 25

# Safety behavior for web driving (recommended True)
...
def send_control(s: dict):
    """
    Translate button booleans into PWM tick outputs (similar spirit to pwm.py).
    """
    if pwm is None:
        return
    ...
    if s.get("brake", False):
        ...
        # Optional: hard limits so web driving can’t run away too far
        # (keeps within your known REV/STOP/FWD band)
    ...
    if s.get("center", False):
        ...
        if left and not right:
            # If your servo is reversed, swap + and - here.
...
if __name__ == "__main__":
    # Ensure we stop the motor on Ctrl+C / kill
    signal.signal(signal.SIGINT, _handle_exit)
    signal.signal(signal.SIGTERM, _handle_exit)

    # Initialize PWM before starting threads
    pwm_init()

    # Start camera worker (ONE ffmpeg for all clients)
    ...
    # Important: disable reloader to avoid starting two copies accidentally
