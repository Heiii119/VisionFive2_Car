...
<head>
  ...
  <title>META DOT FPV Drive + Line Follow (Lab)</title>
  <style>
...
<body>
  <div class="wrap">
    <header>
      <div>
        <div><strong>FPV Drive + Line Follow (Lab)</strong></div>
    ...
    <div class="main">
      ...
      <div class="telemetry">
          ...
          <div class="kv"><span class="muted">Line Lab</span><span id="lineLab">—</span></div>
          <div class="kv"><span class="muted">Lab range</span><span id="labRange">—</span></div>
...
(() => {
  ...
  const lineLabEl = document.getElementById("lineLab");
  const labRangeEl = document.getElementById("labRange");
  ...
  async function pollStatus(){
    try{
      ...
      if (Array.isArray(s.lab_mean)) {
        lineLabEl.textContent = `L:${s.lab_mean[0]}  a:${s.lab_mean[1]}  b:${s.lab_mean[2]}`;
      } else {
        lineLabEl.textContent = "—";
      }

      if (Array.isArray(s.lab_lower) && Array.isArray(s.lab_upper)) {
        labRangeEl.textContent = `[${s.lab_lower.join(", ")}] → [${s.lab_upper.join(", ")}]`;
      } else {
        labRangeEl.textContent = "—";
      }
    }catch(e){
      // ignore
    }finally{
...
def compute_line_error_and_mask_info(bgr, lab_lower, lab_upper):
    lab = cv2.cvtColor(bgr, cv2.COLOR_BGR2LAB)

    lower = np.array(lab_lower, dtype=np.uint8)
    upper = np.array(lab_upper, dtype=np.uint8)
...
def autopilot_loop():
    ...
    while True:
        ...
        with auto_lock:
            ...
            lab_lower = auto_state["lab_lower"]
            lab_upper = auto_state["lab_upper"]
        ...
        if not calibrated or lab_lower is None or lab_upper is None:
            with auto_lock:
        ...
        found, err, cx, roi_w = compute_line_error_and_mask_info(frame, lab_lower, lab_upper)
        now = time.perf_counter()
...
def api_status():
    ...
    return jsonify(
        ...
        # expose calibration Lab info
        lab_mean=s.get("lab_mean"),
        lab_lower=s.get("lab_lower"),
        lab_upper=s.get("lab_upper"),
    )
...
def api_calibrate_pick():
    ...
    # average a small patch around tap to reduce noise
    r = 6
    ...
    L = int(np.mean(lab_patch[:, :, 0]))
    a = int(np.mean(lab_patch[:, :, 1]))
    b = int(np.mean(lab_patch[:, :, 2]))

    lower = lab_clip(L - LAB_TOL_L, a - LAB_TOL_A, b - LAB_TOL_B)
    upper = lab_clip(L + LAB_TOL_L, a + LAB_TOL_A, b + LAB_TOL_B)

    with auto_lock:
        auto_state["lab_mean"] = [L, a, b]
